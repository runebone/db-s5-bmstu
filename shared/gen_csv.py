...
create_users(int(N / 10), user_ids, file_ids)

# Создаём 100 файлов, созданных первыми 100 пользователями
create_files(int(N / 10), user_ids, file_ids)

# Создаём оставшихся 900 пользователей и файлов
create_users(int(N / 10 * 9), user_ids, file_ids)
create_files(int(N / 10 * 9), user_ids, file_ids)
...
# Создаём 1000 постов
create_posts(N, user_ids, post_ids)
...
create_post_attachments(N, post_ids, file_ids)
...
create_post_comments(N, user_ids, post_ids)
...
create_subscriptions(N, user_ids)
